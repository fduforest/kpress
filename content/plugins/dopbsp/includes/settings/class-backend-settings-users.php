                ...
                foreach ($users as $user){
                    ...
                    if ($calendar_id == 0){
                        if ($user->roles[0] == 'administrator'){
                            ...
                            array_push($checkbox_view, '    <input type="checkbox" name="DOPBSP-settings-users-permissions-view-'.$user->ID.'" id="DOPBSP-settings-users-permissions-view-'.$user->ID.'" onclick="DOPBSPBackEndSettingsUsers.set('.$user->ID.', \'view\')" '.($this->permission($user->ID, 'view-all-calendars', 0) ? 'checked="checked"':'').' />');
                            array_push($checkbox_view, '</div>');
                        }
                        else{
                            array_push($checkbox_use, '<div class="dopbsp-input-wrapper">');
                            array_push($checkbox_use, '     <input type="checkbox" name="DOPBSP-settings-users-permissions-use-'.$user->ID.'" id="DOPBSP-settings-users-permissions-use-'.$user->ID.'" onclick="DOPBSPBackEndSettingsUsers.set('.$user->ID.', \'use\')" '.($this->permission($user->ID, 'use-booking-system', 0) ? 'checked="checked"':'').' />');
