...
define('MAPSVG_VERSION', '1.4.6');
define('MAPSVG_JQUERY_VERSION', '5.5.5');
...
function mapsvg_config_page() {
	...
	if ( function_exists('add_submenu_page') )
		$mapsvg_settings_page = add_submenu_page('plugins.php', 'MapSVG', 'MapSVG', 'manage_options', 'mapsvg-config', 'mapsvg_conf');
...
function mapsvg_conf(){
    ...
    if($file){
        ...
        if(!isset($data['m']['width']) || empty($data['m']['width']))
            $data['m']['width'] = $default['m']['width'];
        if(!isset($data['m']['height']) || empty($data['m']['height']))
            $data['m']['height'] = $default['m']['height'];
        if(!isset($data['m']['viewBox']) || empty($data['m']['viewBox']))
            $data['m']['viewBox'] = $default['m']['viewBox'];

        if(!isset($data['m']['colors']) || empty($data['m']['colors']))
