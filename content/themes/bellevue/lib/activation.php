...
function roots_theme_activation_options_add_page() {
    ...
    if (!$roots_activation_options) {
        $theme_page = add_theme_page(
            esc_html__('Theme Activation', 'BELLEVUE'),
            esc_html__('Theme Activation', 'BELLEVUE'),
            'edit_theme_options',
...
function roots_theme_activation_options_render_page() { ?>
    <div class="wrap">
        <h2><?php printf(esc_html__('%s Theme Activation', 'BELLEVUE'), wp_get_theme()); ?></h2>
        <!--div class="update-nag">
      <?php esc_html_e('These settings are optional and should usually be used only on a fresh installation', 'BELLEVUE'); ?>
    </div-->
        <?php //settings_errors(); ?>
        <div class="theme-activation-options theme-activation-step-one">
            <h2><?php esc_html_e('1 - Plugins', 'BELLEVUE'); ?></h2>
            <?php
            ...
            if(!$step_1_tgmpa_not_complete){ ?>
                <p class="dashicons-before dashicons-yes"><?php esc_html_e('Completed', 'BELLEVUE'); ?></p>
            <?php } else { ?>
                <p class="dashicons-before dashicons-plus"><?php printf(wp_kses_post(__('<strong>%1$sInstall and Activate%2$s</strong>, required plugins before continuing.', 'BELLEVUE')), '<a href="'.get_admin_url().'themes.php?page=tgmpa-install-plugins'.'">','</a>'); ?></p>
            <?php }
            ?>
        </div>
        <div class="theme-activation-options theme-activation-step-two">
            <h2><?php esc_html_e('2 - Demo Content', 'BELLEVUE'); ?></h2>
            ...
            if (isset($themo_demo_content_import_completed) && $themo_demo_content_import_completed == 1){ ?>
                <p class="dashicons-before dashicons-yes"><?php esc_html_e('Demo content import completed', 'BELLEVUE'); ?></p>
            <?php }else{ ?>
                <p class="dashicons-before dashicons-plus"><?php printf(wp_kses_post(__('<strong>%1$sImport Demo Content%2$s</strong>  - "%3$s", included in theme zip.', 'BELLEVUE')), '<a href="'.get_admin_url().'admin.php?import=wordpress'.'">','</a>',wp_get_theme().'-demo-content.xml'); ?></p>
                <p style="padding:10px; background-color:#e7e7e7; max-width:600px" class="dashicons-before dashicons-clock"><?php printf(wp_kses_post(__('Importing  takes time. %sLearn more.%s', 'BELLEVUE')), '<a href="http://docs.themovation.com/bellevue/#democontent'.'" target="_blank">','</a></em>'); ?></p>
            <?php }
            ?>
            ...
            if (isset($themo_demo_widget_import_completed) && $themo_demo_widget_import_completed == 1){ ?>
                <p class="dashicons-before dashicons-yes"><?php esc_html_e('Demo widget import completed', 'BELLEVUE'); ?></p>
            <?php }else{ ?>
                <p class="dashicons-before dashicons-plus"><?php printf(wp_kses_post(__('<strong>%1$sImport Widget Content%2$s </strong> - "%3$s", includced in theme zip.', 'BELLEVUE')), '<a href="'.get_admin_url().'tools.php?page=widget-importer-exporter'.'">','</a>',wp_get_theme().'-demo-widget-content.wie'); ?></p>
            <?php } ?>
            ...
            if (isset($themo_demo_form_import_completed) && $themo_demo_form_import_completed == 1){ ?>
                <p class="dashicons-before dashicons-yes"><?php esc_html_e('Demo form import completed', 'BELLEVUE'); ?></p>
            <?php }else{ ?>
                <p class="dashicons-before dashicons-plus"><?php printf(wp_kses_post(__('<strong>%1$sImport Forms%2$s</strong> - "%3$s", includced in theme zip.', 'BELLEVUE')), '<a href="'.get_admin_url().'admin.php?page=formidable-import'.'">','</a>',wp_get_theme().'-formidable-custom-forms.xml'); ?></p>
            <?php } ?>
        ...
        <div class="theme-activation-options theme-activation-step-three">
            <h2><?php esc_html_e('3 - Settings', 'BELLEVUE'); ?></h2>
            ...
            if (isset($themo_theme_activation_completed) && $themo_theme_activation_completed == 1){ ?>
                <p class="dashicons-before dashicons-yes"><?php esc_html_e('Settings Step Completed', 'BELLEVUE'); ?></p>
            ...
            <?php }else{ ?>
                ...
                <form method="post" action="options.php">
                    ...
                    <table class="form-table">
                        ...
                        <?php
                        $page_names = array('Home'); // list of all possible page names.
                        ...
                        if(isset($page_home->post_title) && $page_home->post_title > ""){ ?>
                            <tr valign="top"><th scope="row"><?php esc_html_e('Set static Front page?', 'BELLEVUE'); ?></th>
                                <td>
                                    <fieldset>
                                        <legend class="screen-reader-text"><span><?php esc_html_e('Set a static Front page?', 'BELLEVUE'); ?></span></legend>
                                        <?php
                                        $args = array(
                                            'sort_order' => 'ASC',
                                        ...
                                        $option_output = '<option value="false">'. esc_html__('No', 'BELLEVUE') . '</option>';
                                        ...
                                        <p class="description"><?php printf(esc_html__('Set a page it to be the static Front page.', 'BELLEVUE')); ?></p>
                                    </fieldset>
                                </td>
                            </tr>
                        <?php }else{ ?>

                            <tr valign="top"><th scope="row"><?php esc_html_e('Create Front page?', 'BELLEVUE'); ?></th>
                                <td>
                                    <fieldset>
                                        <legend class="screen-reader-text"><span><?php esc_html_e('Create Front page?', 'BELLEVUE'); ?></span></legend>
                                        <select name="roots_theme_activation_options[create_front_page]" id="create_front_page">
                                            <option selected="selected" value="true"><?php echo esc_html_e('Yes', 'BELLEVUE'); ?></option>
                                            <option value="false"><?php echo esc_html_e('No', 'BELLEVUE'); ?></option>
                                        </select>
                                        <p class="description"><?php printf(esc_html__('Create a page called Home and set it to be the Front page.', 'BELLEVUE')); ?></p>
                                    </fieldset>
                                </td>
                            </tr>
                        <?php } ?>
                        <?php if(get_page_by_title('blog')){ ?>
                            <!--tr valign="top"><th scope="row"><?php esc_html_e('Set Post page?', 'BELLEVUE'); ?></th>
                                <td>
                                    <fieldset>
                                        <legend class="screen-reader-text"><span><?php esc_html_e('Set a Post page to be your default blog.', 'BELLEVUE'); ?></span></legend>
                                        <?php
                                        $args = array(
                                            'sort_order' => 'ASC',
                                            'sort_column' => 'post_title',
                                        ...
                                        <p class="description"><?php printf(esc_html__('Set a page it to be the Post page (default blog)', 'BELLEVUE')); ?></p>
                                    </fieldset>
                                </td>
                            </tr-->
                        <?php }else{ ?>

                            <!--tr valign="top"><th scope="row"><?php esc_html_e('Create Post page?', 'BELLEVUE'); ?></th>
                                <td>
                                    <fieldset>
                                        <legend class="screen-reader-text"><span><?php esc_html_e('Create Post page?', 'BELLEVUE'); ?></span></legend>
                                        <select name="roots_theme_activation_options[create_post_page]" id="create_post_page">
                                            <option selected="selected" value="true"><?php echo esc_html_e('Yes', 'BELLEVUE'); ?></option>
                                            <option value="false"><?php echo esc_html_e('No', 'BELLEVUE'); ?></option>
                                        </select>
                                        <p class="description"><?php printf(esc_html__('Create a page called Blog and set it to be the Post page', 'BELLEVUE')); ?></p>
                                    </fieldset>
                                </td>
                            </tr-->
                        <?php } ?>
                        <tr valign="top"><th scope="row"><?php esc_html_e('Switch to Search Engine Friendly URLs?', 'BELLEVUE'); ?></th>
                            <td>
                                <fieldset>
                                    <legend class="screen-reader-text"><span><?php esc_html_e('Update permalink structure?', 'BELLEVUE'); ?></span></legend>
                                    <select name="roots_theme_activation_options[change_permalink_structure]" id="change_permalink_structure">
                                        <option selected="selected" value="true"><?php echo esc_html_e('Yes', 'BELLEVUE'); ?></option>
                                        <option value="false"><?php echo esc_html_e('No', 'BELLEVUE'); ?></option>
                                    </select>
                                    <p class="description"><?php printf(wp_kses_post(__('Change permalink structure to /&#37;postname&#37;/', 'BELLEVUE'))); ?></p>
                                </fieldset>
                            </td>
                        </tr>
                        <tr valign="top"><th scope="row"><?php esc_html_e('Create navigation menu?', 'BELLEVUE'); ?></th>
                            <td>
                                <fieldset>
                                    <legend class="screen-reader-text"><span><?php esc_html_e('Create navigation menu?', 'BELLEVUE'); ?></span></legend>
                                    <select name="roots_theme_activation_options[create_navigation_menus]" id="create_navigation_menus">
                                        <option selected="selected" value="true"><?php echo esc_html_e('Yes', 'BELLEVUE'); ?></option>
                                        <option value="false"><?php echo esc_html_e('No', 'BELLEVUE'); ?></option>
                                    </select>
                                    <p class="description"><?php printf(esc_html__('Setup Primary Navigation', 'BELLEVUE')); ?></p>
                                </fieldset>
                            </td>
                        </tr>
                        <tr valign="top"><th scope="row"><?php esc_html_e('Add pages to menu?', 'BELLEVUE'); ?></th>
                            <td>
                                <fieldset>
                                    <legend class="screen-reader-text"><span><?php esc_html_e('Add pages to menu?', 'BELLEVUE'); ?></span></legend>
                                    <select name="roots_theme_activation_options[add_pages_to_primary_navigation]" id="add_pages_to_primary_navigation">
                                        <option selected="selected" value="true"><?php echo esc_html_e('Yes', 'BELLEVUE'); ?></option>
                                        <option value="false"><?php echo esc_html_e('No', 'BELLEVUE'); ?></option>
                                    </select>
                                    <p class="description"><?php printf(esc_html__('Add all current published pages to the Primary Navigation', 'BELLEVUE')); ?></p>
                                </fieldset>
                            </td>
                        </tr>
                    </table>
...
//-----------------------------------------------------
    ...
    if (isset($roots_theme_activation_options['create_front_page']) && $roots_theme_activation_options['create_front_page'] === 'true') {
        ...
        $default_pages = array(esc_html__('Home 1', 'BELLEVUE'));
        ...
        $home = get_page_by_title(esc_html__('Home', 'BELLEVUE'));
...
//-----------------------------------------------------
    ...
    if (isset($roots_theme_activation_options['create_post_page']) && $roots_theme_activation_options['create_post_page'] === 'true') {
        ...
        $default_pages = array(esc_html__('Blog', 'BELLEVUE'));
        $existing_pages = get_pages();
        $temp = array();
        ...
        // Name of Page
        $blog = get_page_by_title(esc_html__('Blog', 'BELLEVUE'));
...
//-----------------------------------------------------
    ...
    if ($roots_theme_activation_options['create_navigation_menus'] === 'true') {
        ...
        $primary_nav = wp_get_nav_menu_object(esc_html__('Primary Navigation', 'BELLEVUE'));

        if (!$primary_nav) {
            $primary_nav_id = wp_create_nav_menu(esc_html__('Primary Navigation', 'BELLEVUE'), array('slug' => 'primary_navigation'));
...
function themo_activation_admin_notice() {
    ...
    if ( ! get_user_meta($user_id, 'themo_activation_ignore_notice') && ($pagenow == 'themes.php' || $pagenow == 'tools.php' || $pagenow == 'admin.php' || $pagenow == 'plugins.php')) {
        ...
        printf(wp_kses_post(__('<a href="%2$s">Back to Theme Activation</a> (not completed yet) | <a href="%1$s"><small><em>dismiss</em></small></a>', 'BELLEVUE')), '?themo_activation_nag_ignore=0',admin_url('themes.php?page=theme_activation_options'));
        echo "</div>";
    }
}
