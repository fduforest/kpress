...
function themo_return_cpt_args($postid='',$key='',$post_type='',$taxonomy=''){
    ...
    // No post limit per page, except for themo rooms which will use the default.
    if (isset($post_type) && $post_type == 'themo_rooms') {
    }else{
...
//-----------------------------------------------------
// themo_print_room_icon
// Print mar-up for room icons
//-----------------------------------------------------

function themo_print_room_icon($id = false, $custom_glyphicon = false, $glyphicon_set = false) {
	...
	$room_icon_support = 'on'; //
	if ( function_exists( 'ot_get_option' ) ) {
		$room_icon_support = ot_get_option( 'themo_room_icons', 'on' );
	}

	$room_icon_display = get_post_meta($id, 'themo_room_show_icon');
	if(isset($room_icon_display[0]) && $room_icon_display[0] == 1 && $room_icon_support == 'on') {
		$room_icon = '<i class="room-icon '.$glyphicon_set.' '.$custom_glyphicon.'"></i>';
	}else {
		$room_icon = "";
	}
	echo wp_kses_post($room_icon);
}
...
function themo_custom_excerpt_more( $output ) {
	if ( (has_excerpt() || themo_has_more()) && ! is_attachment() && get_post_type() != 'themo_rooms') {
...
function themo_rooms_template_options( $query ) {
...
//add_action( 'pre_get_posts', 'themo_rooms_template_options', 1 );
...
//Include the AddThis meta box to rooms.
//-----------------------------------------------------
 ...
 * - change 'page-rooms.php' to your template's filename
...
function themo_wpse_50092_script_enqueuer() {
	...
	echo <<<HTML
		...
		"use strict";
        var excludeTemplates = [ "templates/rooms-standard.php","templates/blog-masonry.php","templates/blog-standard.php"];
        ...
        var excludeFound = $.inArray(currentTemplate, excludeTemplates);
            ...
            */
            if( excludeFound !== -1 && !excludeFound > -1 || $iswooshoppage) {
                ...
                $('#themo_rooms_options_meta_box').hide();
            }
            if( currentTemplate ==  "templates/rooms-standard.php") {
            	$('#themo_rooms_options_meta_box').show();
            }else{
            	$('#themo_rooms_options_meta_box').hide();
            }
...
function themo_ot_meta_box_post_format_link($array,$pages) {
	...
	$array['fields'] = array(
		...
		array(
			...
			'label'       => esc_html__( 'Link behaviour on the rooms homepage', 'BELLEVUE' ),
			'desc' => 'By default the rooms homepage thumbnail goes to the room single page. You can make this link go directly to your the URL above by using this checkbox.',
			'type'        => 'checkbox',
			'choices'     => array(
				array(
					'value'       => true,
					'label'       => 'Take user directly to URL above &amp; skip room single.',
				)
			)
		)
	);
...
function themo_filter_ot_recognized_background_repeat( $array, $field_id ) {
	$array = array(
		'no-repeat' => esc_html__( 'No Repeat', 'BELLEVUE' ),
		'repeat'    => esc_html__( 'Repeat All', 'BELLEVUE' ),
		'repeat-x'  => esc_html__( 'Repeat Horizontally', 'BELLEVUE' ),
		'repeat-y'  => esc_html__( 'Repeat Vertically', 'BELLEVUE' ),
	);
...
function themo_filter_ot_recognized_background_attachment( $array, $field_id ) {
	$array = array(
		'fixed' => esc_html__( 'Fixed', 'BELLEVUE' ),
		'scroll'    => esc_html__( 'Scroll', 'BELLEVUE' ),
	);
...
function themo_register_required_plugins() {
	...
	$plugins = array(
		...
		array(
			'name'               => 'Bellevue Shortcodes', // The plugin name.
			'slug'               => 'bellevue-shortcodes', // The plugin slug (typically the folder name).
			'source'             => get_stylesheet_directory() . '/plugins/bellevue-shortcodes.zip', // The plugin source.
		...
		array(
			'name'               => 'Bellevue Custom Post Types', // The plugin name.
			'slug'               => 'bellevue-custom-post-types', // The plugin slug (typically the folder name).
			'source'             => get_stylesheet_directory() . '/plugins/bellevue-custom-post-types.zip', // The plugin source.
			'required'           => true, // If false, the plugin is only 'recommended' instead of required.
        ...
        array(
            ...
            'required'           => true, // If false, the plugin is only 'recommended' instead of required.
            'version'            => '4.7.3', // E.g. 1.0.0. If set, the active plugin must be this version or higher.
            'force_activation'   => true, // If true, plugin is activated upon theme activation and cannot be deactivated until theme switch.
            'force_deactivation' => false, // If true, plugin is deactivated upon theme switch, useful for theme-specific plugins.
        ),
        array(
            ...
            'required'           => true, // If false, the plugin is only 'recommended' instead of required.
            'version'            => '0.1.5', // E.g. 1.0.0. If set, the active plugin must be this version or higher.
            'force_activation'   => true, // If true, plugin is activated upon theme activation and cannot be deactivated until theme switch.
            'force_deactivation' => false, // If true, plugin is deactivated upon theme switch, useful for theme-specific plugins.
            'external_url'       => '', // If set, overrides default API URL and points to an external URL.
        ),
		array(
			...
			'source'             => get_stylesheet_directory() . '/plugins/wp-booking-system.zip', // The plugin source.
        ...
        array(
            ...
            'required'           => false, // If false, the plugin is only 'recommended' instead of required.
            'version'            => '2.26.0', // E.g. 1.0.0. If set, the active plugin must be this version or higher.
            'force_activation'   => false, // If true, plugin is activated upon theme activation and cannot be deactivated until theme switch.
            'force_deactivation' => false, // If true, plugin is deactivated upon theme switch, useful for theme-specific plugins.
            'external_url'       => '', // If set, overrides default API URL and points to an external URL.
        ),
	...
	$config = array(
		...
		'strings'      => array(
			'page_title'                      => esc_html__( 'Install Required Plugins', 'BELLEVUE' ),
			'menu_title'                      => esc_html__( 'Install Plugins', 'BELLEVUE' ),
			'installing'                      => esc_html__( 'Installing Plugin: %s', 'BELLEVUE' ), // %s = plugin name.
			'oops'                            => esc_html__( 'Something went wrong with the plugin API.', 'BELLEVUE' ),
			'notice_can_install_required'     => _n_noop( 'This theme requires the following plugin: %1$s.', 'This theme requires the following plugins: %1$s.', 'BELLEVUE' ), // %1$s = plugin name(s).
			'notice_can_install_recommended'  => _n_noop( 'This theme recommends the following plugin: %1$s.', 'This theme recommends the following plugins: %1$s.', 'BELLEVUE' ), // %1$s = plugin name(s).
			'notice_cannot_install'           => _n_noop( 'Sorry, but you do not have the correct permissions to install the %s plugin. Contact the administrator of this site for help on getting the plugin installed.', 'Sorry, but you do not have the correct permissions to install the %s plugins. Contact the administrator of this site for help on getting the plugins installed.', 'BELLEVUE' ), // %1$s = plugin name(s).
			'notice_can_activate_required'    => _n_noop( 'The following required plugin is currently inactive: %1$s.', 'The following required plugins are currently inactive: %1$s.', 'BELLEVUE' ), // %1$s = plugin name(s).
			'notice_can_activate_recommended' => _n_noop( 'The following recommended plugin is currently inactive: %1$s.', 'The following recommended plugins are currently inactive: %1$s.', 'BELLEVUE' ), // %1$s = plugin name(s).
			'notice_cannot_activate'          => _n_noop( 'Sorry, but you do not have the correct permissions to activate the %s plugin. Contact the administrator of this site for help on getting the plugin activated.', 'Sorry, but you do not have the correct permissions to activate the %s plugins. Contact the administrator of this site for help on getting the plugins activated.', 'BELLEVUE' ), // %1$s = plugin name(s).
			'notice_ask_to_update'            => _n_noop( 'The following plugin needs to be updated to its latest version to ensure maximum compatibility with this theme: %1$s.', 'The following plugins need to be updated to their latest version to ensure maximum compatibility with this theme: %1$s.', 'BELLEVUE' ), // %1$s = plugin name(s).
			'notice_cannot_update'            => _n_noop( 'Sorry, but you do not have the correct permissions to update the %s plugin. Contact the administrator of this site for help on getting the plugin updated.', 'Sorry, but you do not have the correct permissions to update the %s plugins. Contact the administrator of this site for help on getting the plugins updated.', 'BELLEVUE' ), // %1$s = plugin name(s).
			'install_link'                    => _n_noop( 'Begin installing plugin', 'Begin installing plugins', 'BELLEVUE' ),
			'activate_link'                   => _n_noop( 'Begin activating plugin', 'Begin activating plugins', 'BELLEVUE' ),
			'return'                          => esc_html__( 'Return to Required Plugins Installer', 'BELLEVUE' ),
			'plugin_activated'                => esc_html__( 'Plugin activated successfully.', 'BELLEVUE' ),
			'complete'                        => esc_html__( 'All plugins installed and activated successfully. %s', 'BELLEVUE' ), // %s = dashboard link.
...
function themo_import_end(){
    ...
    foreach ( $pages as $page ) {
        //$page->ID;
            switch ($page->post_title) {
                case 'Home':
                    update_post_meta($page->ID, 'themo_slider_flex_groups', term_exists('homepage'));
                    update_post_meta($page->ID, 'themo_thumb_slider_1_groups', term_exists('homepage'));
                    update_post_meta($page->ID, 'themo_rooms_1_groups', term_exists('en-suite'));
                    update_post_meta($page->ID, 'themo_tour_1_groups', term_exists('homepage'));
                    update_post_meta($page->ID, 'themo_testimonials_1_groups', term_exists('homepage'));
                    update_post_meta($page->ID, 'themo_featured_1_groups', term_exists('homepage'));
                    break;
                case 'Availability':
                    update_post_meta($page->ID, 'themo_showcase_1_groups', term_exists('availability'));
                    update_post_meta($page->ID, 'themo_showcase_2_groups', term_exists('availability'));
                    update_post_meta($page->ID, 'themo_showcase_3_groups', term_exists('availability'));
                    break;
                case 'Contact':
                    update_post_meta($page->ID, 'themo_service_block_1_groups', term_exists('contact'));
                    update_post_meta($page->ID, 'themo_service_block_split_1_groups', term_exists('explore'));
                    update_post_meta($page->ID, 'themo_faq_1_groups', term_exists('contact'));
                    update_post_meta($page->ID, 'themo_brands_1_groups', term_exists('contact'));
                    break;
                case 'Explore':
                    update_post_meta($page->ID, 'themo_service_block_1_groups', term_exists('explore-what-to-do'));
                    update_post_meta($page->ID, 'themo_tour_1_groups', term_exists('explore-what-to-do'));
                    update_post_meta($page->ID, 'themo_tour_2_groups', term_exists('explore-what-to-see'));
                    update_post_meta($page->ID, 'themo_tour_3_groups', term_exists('explore-where-to-eat'));
                    update_post_meta($page->ID, 'themo_service_block_split_1_groups', term_exists('explore'));
                    break;
                case 'Our Menu':
                    update_post_meta($page->ID, 'themo_thumb_slider_1_groups', term_exists('menu'));
                    update_post_meta($page->ID, 'themo_service_block_1_groups', term_exists('menu'));
                    update_post_meta($page->ID, 'themo_service_block_1_groups', term_exists('menu'));
                    update_post_meta($page->ID, 'themo_service_block_split_1_groups', term_exists('explore'));
                    update_post_meta($page->ID, 'themo_brands_1_groups', term_exists('contact'));
                    break;
                case 'Our Place':
                    update_post_meta($page->ID, 'themo_service_block_1_groups', term_exists('our-place'));
                    update_post_meta($page->ID, 'themo_pricing_plans_1_groups', term_exists('our-place'));
                    update_post_meta($page->ID, 'themo_brands_1_groups', term_exists('contact'));
                    update_post_meta($page->ID, 'themo_testimonials_1_groups', term_exists('homepage'));
                    break;
                case 'Packages and Specials':
                    update_post_meta($page->ID, 'themo_featured_1_groups', term_exists('homepage'));
                    update_post_meta($page->ID, 'themo_featured_2_groups', term_exists('packages-and-specials'));
                    update_post_meta($page->ID, 'themo_testimonials_1_groups', term_exists('single'));
                    update_post_meta($page->ID, 'themo_service_block_split_1_groups', term_exists('explore'));
                    break;
                case 'Rooms and Suites':
                    update_post_meta($page->ID, 'themo_rooms_1_groups', term_exists('king').','.term_exists('queen'));
                    update_post_meta($page->ID, 'themo_rooms_2_groups', term_exists('king').','.term_exists('queen').','.term_exists('single-bed'));
                    update_post_meta($page->ID, 'themo_rooms_3_groups', term_exists('king').','.term_exists('kitchenette').','.term_exists('single-bed'));
                    update_post_meta($page->ID, 'themo_testimonials_1_groups', term_exists('packages-and-specials'));
                    break;
                case 'Why Us':
                    update_post_meta($page->ID, 'themo_thumb_slider_1_groups', term_exists('why-us'));
                    update_post_meta($page->ID, 'themo_team_1_groups', term_exists('why-us'));
                    update_post_meta($page->ID, 'themo_brands_1_groups', term_exists('contact'));
                    update_post_meta($page->ID, 'themo_testimonials_1_groups', term_exists('single'));
                    break;
        }
	...
	foreach ( $posts as $post ) {
		switch ($post->post_title) {
			case 'Newcastle':
				update_post_meta($post->ID, 'themo_service_block_1_groups', term_exists('room-single'));
				update_post_meta($post->ID, 'themo_pricing_plans_1_groups', term_exists('our-place'));
				update_post_meta($post->ID, 'themo_testimonials_1_groups', term_exists('single'));
				break;
			case 'Tofino':
				update_post_meta($post->ID, 'themo_service_block_1_groups', term_exists('room-single'));
				update_post_meta($post->ID, 'themo_pricing_plans_1_groups', term_exists('our-place'));
